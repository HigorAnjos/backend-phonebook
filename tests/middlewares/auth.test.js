...
describe('Error na decoded ', () => { 
  ...
  before(() => {
    ...
    sinon.stub(jwt, 'verify').throws('Error em verify');
  })
  ...
  // it('A requisicao deve retornar obj { message: "Token inválido", error: Error em verify }', () => {
  //   auth(request, response, next);
    
  //   expect(response.json.calledWith({ message: 'Token inválido', error: 'Error em verify' }))
  //   .to.be.equal(true); // deveria parar a funcao
  // })
})
